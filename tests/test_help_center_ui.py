...
class HelpCenterUITests(unittest.TestCase):
    ...
    def test_styles_import_only_the_selected_local_tokens(self):
        ...
        retired_cards = {'--bee-card-background', '--bee-card-border',
    ...
    def test_home_and_category_routes(self):
        ...
        self.assertIn('version: 1.2', home)
        self.assertIn('displayed_sidebar: docsSidebar', home)
        self.assertIn('slug: /', home)
        self.assertIn('# ¿Cómo podemos ayudarte?', home)
        self.assertNotIn('help-categories', home)
        self.assertNotIn('Explora por categoría', home)
        sidebars = (ROOT / 'sidebars.js').read_text(encoding='utf-8')
        self.assertIn("docsSidebar: [{type: 'autogenerated', dirName: 'centro-de-ayuda'}]", sidebars)
        categories = []
        ...
        self.assertEqual(list(range(1, 9)), [position for position, _ in sorted(categories)])
